...
tags:
  ...
  - 젭바운드
  - 오포글리프론
description: "정재훈 약사가 설명한 GLP-1 비만치료제의 가격 구조, 위고비와 마운자로의 차이, 먹는 비만약의 확산 가능성, 영양·근손실 관리 포인트를 구조적으로 정리합니다."
---
...
```mermaid
flowchart TD
    A["당뇨 치료제 사용"] --> B["체중 감소 관찰"]
    B --> C["비만 치료 적응증 확대"]
    C --> D["심혈관 위험 감소 적응증"]
    C --> E["수면무호흡증 등 추가 적응증 탐색"]
    E --> F["체중감량 + 직접 약리 효과 분리 연구"]
...
flowchart TD
    A["주사형 비만약"] --> B["강한 약효"]
    A --> C["주사 부담\n보관 부담"]
    D["경구형 펩타이드"] --> E["복용 편의 증가"]
    D --> F["흡수율 제약\n복용 조건 까다로움"]
    G["경구형 소분자\norforglipron 계열"] --> H["복용 자유도 확대 기대"]
    H --> I["시장 대중화 가능성"]
    C --> I
    E --> I
...
```mermaid
flowchart TD
    A["비만 환자"] --> B["위 포만감 신호 문제"]
    A --> C["뇌 보상회로/food noise 문제"]
    A --> D["감정적 섭식"]
    A --> E["정신과 약물로 인한 체중 증가"]
    B --> F["GLP-1 계열에 잘 반응할 수 있음"]
    C --> G["부분 반응 또는 추가 표적 필요"]
    D --> G
    E --> H["보조 치료로 활용 가능"]
...
```mermaid
flowchart TD
    A["비만약 대중화"] --> B["체중 관리 방식 변화"]
    B --> C["처방/보험/직배송 구조 변화"]
    B --> D["외식 메뉴 소형화"]
    B --> E["고단백 식품 수요 증가"]
    B --> F["빅사이즈 패션 시장 압박"]
    E --> G["운동 + 영양 관리의 중요성 재강조"]
